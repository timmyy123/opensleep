...
public final class GoalRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public final List<AbstractGoalItem<?>> getItems() {
        return this.items;
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        holder.getClass();
        AbstractGoalItem<?> abstractGoalItem = this.items.get(position);
        abstractGoalItem.getClass();
        abstractGoalItem.bindView(holder);
    }

    @Override // androidx.recyclerview.widget.RecyclerView.Adapter
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        parent.getClass();
        for (AbstractGoalItem<?> abstractGoalItem : this.items) {
            if (abstractGoalItem.getType().ordinal() == viewType) {
                return abstractGoalItem.getViewHolder(parent);
            }
        }
        return new GoalItem(this.context, new Goal(0.0d, Goal.Type.DURATION, 0L, 0L, 12, null)).createViewHolder((View) parent);
    }
}
