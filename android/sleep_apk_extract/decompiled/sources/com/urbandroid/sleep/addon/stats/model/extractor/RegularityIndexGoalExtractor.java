...
public class RegularityIndexGoalExtractor implements IValueExtractor {
    private Context context;

    public RegularityIndexGoalExtractor(Context context) {
    ...
    public String getValuePresentation(double d) {
        return d == -1.0d ? "" : DecimalFormatUtil.getZeroDigit().format(d * 100.0d);
    }

    @Override // com.urbandroid.sleep.addon.stats.model.extractor.IValueExtractor
    public double parseValueFromPresentation(String str) {
        Logger.logDebug("GOAL: " + str);
        if ("".equals(str)) {
            return -1.0d;
        }
