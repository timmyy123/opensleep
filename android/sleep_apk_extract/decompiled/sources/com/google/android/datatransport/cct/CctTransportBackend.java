...
final class CctTransportBackend implements TransportBackend {
    ...
    private BatchedLogRequest getRequestBody(BackendRequest backendRequest) {
        ...
        for (Map.Entry entry : map.entrySet()) {
            ...
            LogRequest.Builder clientInfo = LogRequest.builder().setQosTier(QosTier.DEFAULT).setRequestTimeMs(this.wallTimeClock.getTime()).setRequestUptimeMs(this.uptimeClock.getTime()).setClientInfo(ClientInfo.builder().setClientType(ClientInfo.ClientType.ANDROID_FIREBASE).setAndroidClientInfo(AndroidClientInfo.builder().setSdkVersion(Integer.valueOf(eventInternal2.getInteger("sdk-version"))).setModel(eventInternal2.get(DeviceRequestsHelper.DEVICE_INFO_MODEL)).setHardware(eventInternal2.get("hardware")).setDevice(eventInternal2.get(DeviceRequestsHelper.DEVICE_INFO_DEVICE)).setProduct(eventInternal2.get("product")).setOsBuild(eventInternal2.get("os-uild")).setManufacturer(eventInternal2.get("manufacturer")).setFingerprint(eventInternal2.get("fingerprint")).setCountry(eventInternal2.get("country")).setLocale(eventInternal2.get(SpotifyService.LOCALE)).setMccMnc(eventInternal2.get("mcc_mnc")).setApplicationBuild(eventInternal2.get("application_build")).build()).build());
            try {
            ...
            for (EventInternal eventInternal3 : (List) entry.getValue()) {
                ...
                if (encoding.equals(Encoding.of("proto"))) {
                    ...
                    builderProtoBuilder = LogEvent.jsonBuilder(new String(encodedPayload.getBytes(), Charset.forName(OAuth.ENCODING)));
                } else {
    ...
    private static URL parseUrlOrThrow(String str) {
        try {
            ...
            throw new IllegalArgumentException(FileInsert$$ExternalSyntheticOutline0.m("Invalid url: ", str), e);
        }
    }
